...
export default function CovidStatus(){
    ...
    const onClick=(e:any)=>{
        e.preventDefault();
        if(selectedOption==='no'||selectedOption==='now'||numbers.number&&numbers.quantity||numbers.date){
            nav('/vaccination')
    ...
    return(
        <>
        <Content click={onLeftArrowClick} leftarrow={Leftarrow} picture={picture} inputs={
            <Form onSubmit={onClick}>
                ...
                <Label>თუ გახსოვს, გთხოვ მიუთითე ტესტის მიახლოებითი რიცხვი და ანტისხეულების რაოდენობა*</Label>
                <InputNum 
                type="number" 
                placeholder="რიცხვი"
                ...
                {numbers.number?null:<ErrorMessage>{errorNumber}</ErrorMessage>}
                <InputNum 
                type="number" 
                ...
                {numbers.quantity?null:<ErrorMessage>{errorNumber}</ErrorMessage>}
                ...
                name="date"
                ></Input>
                {numbers.date?null:<ErrorMessage>{errorNumber}</ErrorMessage>}
                </>:''}
                <Button 
                type="submit">
                <ImgArrow 
                style={{opacity:selectedOption&&select&&numbers.number&&numbers.quantity||numbers.date?'100%':'50%'}}  
                src={arrow}>
                </ImgArrow>
                </Button>
